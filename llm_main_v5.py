...
from llm_sensitivity_pruning import compute_gradient_sensitivity, prune_by_sensitivity
...
def main():
    ...
    # Step 1: Fine-tuning (or Loading) the LLM Model
    print(Fore.YELLOW + "\n🚀 **Step 1: Fine-tuning (or Loading) the LLM Model**" + Style.RESET_ALL)
    model = load_or_train_model(MODEL_PATHS["finetuned"])
    model = model.to(device)  # Ensure the model is on the proper device

    # New Step 1a: Sensitivity-based Pruning (Gradient-based)
    ...
    print("Sensitivity-based pruning applied.")

    # Step 2: Pruning the Model (Existing Method)
